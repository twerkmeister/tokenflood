...
heuristic_tasks = [
    HeuristicTask(task="Ignore the random input and continue the following sequence up to 10000 without abbreviation: 1 2 3 4")
]
...
def create_prompt(token_set: TokenSet, prompt_length: int, prefix_length: int, task: HeuristicTask) -> str:
...
def create_heuristic_messages(heuristic_test_spec: HeuristicTestSpec) -> List[Tuple[MessageList, int]]:
    task = heuristic_tasks[0]
    token_set = heuristic_token_sets[0]
    messages = []
    for prompt_length, prefix_length, output_length in zip(heuristic_test_spec.sample()):

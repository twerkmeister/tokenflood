...
def create_schedule(test_spec: TestSpec) -> List[float]:
    ...
    pauses = np.random.exponential(
        1 / test_spec.requests_per_second, size=test_spec.total_num_requests
    )
...
async def run_test(
    ...
    num_generation_tokens_list: List[int],
    endpoint_spec: EndpointSpec,
) -> List[ModelResponse]:
    requests: List[asyncio.Task] = []
    for pause, messages, num_generation_tokens in tqdm(
        zip(schedule, message_lists, num_generation_tokens_list)
    ):
        requests.append(
            asyncio.create_task(
                send_llm_request(endpoint_spec, messages, num_generation_tokens)
            )
        )

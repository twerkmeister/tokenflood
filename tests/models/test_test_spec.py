...
@pytest.mark.parametrize("name, requests_per_second, test_length_in_seconds, expectation", [
...
def test_test_spec_validation(name, requests_per_second, test_length_in_seconds, expectation):
    with expectation:
        TestSpec(name=name, requests_per_second=requests_per_second,
                          test_length_in_seconds=test_length_in_seconds)



@pytest.mark.parametrize("prompt_lengths, output_lengths, prefix_lengths, expectation", [
...
def test_heuristic_test_spec_validation(prompt_lengths, output_lengths, prefix_lengths, expectation):
    with expectation:
        HeuristicTestSpec(name="abc", requests_per_second=3,
...
def test_heuristic_test_spec_sampling():
    spec = HeuristicTestSpec(name="abc", requests_per_second=3,
                      test_length_in_seconds=10,
    ...
    assert np.allclose(np.average(spec.prompt_lengths), np.average(prompt_lengths), atol=1)
    assert np.allclose(np.average(spec.prefix_lengths), np.average(prefix_lengths), atol=1)
    assert np.allclose(np.average(spec.output_lengths), np.average(output_lengths), atol=1)

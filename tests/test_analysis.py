...
def test_aggregate(llm_requests_df):
    field = "latency"
    assert round(np.average(llm_requests_df[field]), 2) == aggregate(llm_requests_df, field, mean_float)
    assert int(np.average(llm_requests_df[field])) == aggregate(llm_requests_df, field, mean_int)
...
def test_get_group_stats(llm_requests_df):
    ...
    assert group_stats["0"] == [0, .5, 0, 1]
    assert group_stats["1"] == [1, 1.5, 0, 3]


@pytest.mark.parametrize("sequence, percentile, expected_result", [
    ([], 25, 0.0),
    ([], 75, 0.0),
    ([1], 20, 1.0),
    ([1], 80, 1.0),
...
@pytest.mark.parametrize("stats1, stats2, result_stats", [
    ({"a": [1, 2]}, {"a": [3]}, {"a": [1,2,3]}),
